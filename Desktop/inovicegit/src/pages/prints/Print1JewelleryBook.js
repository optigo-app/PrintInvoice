...
export default function Print1JewelleryBook({
  ...
  const handlePrintWithAllData = () => {
    ...
    let attempts = 0;
    const maxAttempts = 200; // ~200 frames (~3s @60fps)
    const waitForDOM = () => {
      requestAnimationFrame(() => {
        attempts++;
        const items = document.querySelectorAll(".col1");
        if (items?.length >= result?.DT?.length || attempts > maxAttempts) {
          handlePrint();
        } else {
          waitForDOM();
        }
          ...
          {visibleItems.map((e, i) => (
            <div key={i} className="col1 brbxAll spfntbH pagBrkIns">
              ...
              <div className="w-100 disflxCen spaclftTpm">
                ...
                <div
                  className={`${
                    e?.Metal_Type ? "wdth_50 spacrighTpm" : "w-100 spfntlft"
                  } spbrWord`}
                >
